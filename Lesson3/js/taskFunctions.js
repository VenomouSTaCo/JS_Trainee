...
console.log(getCodeStringFromText('hello', function (symbol) {
...
console.log(parser(['my', 'name', 'is', 'Trinity'], function (symbol) {
    return symbol[0].toUpperCase() + symbol.slice(1);
}))

console.log(parser([10, 20, 30], function (symbol) {
    return ` ${symbol * 10},`
}))

console.log(parser([{age: 45, name: 'John'}, {age: 20, name: 'Aaron'}], function callback(symbol) {
    return ` ${symbol.name} is ${symbol.age},`
}))

console.log(parser(['abc', '123'], function (symbol) {
    return ` ${symbol.split("").reverse().join("")},`
}))

console.log(parser([1, 2, 3], function (symbol) {
    return ` ${symbol + 5},`
}))
